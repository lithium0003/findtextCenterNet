...
def loss_function(fmask, labelmap, idmap, heatmap, decoder_outputs):
    ...
    mask3 = torch.logical_and(keylabel.flatten()[fmask] > key_th3, idmap[:,0,:,:].flatten()[fmask] > 0)
    mask4 = torch.logical_and(keylabel.flatten()[fmask] == 1, idmap[:,0,:,:].flatten()[fmask] > 0)
    ...
    weight3 = torch.maximum(keylabel.flatten()[fmask] - key_th3, torch.tensor(0.)) / (1 - key_th3)
    weight3 = torch.masked_select(weight3, mask3)
    weight3_count = torch.maximum(torch.tensor(1.), weight3.sum())

    keymap_loss = heatmap_loss(true=keylabel, logits=heatmap[:,0,:,:]) * 10.
...
def loss_function3(outputs, labelcode, mask):
    target_ids = []
    for modulo in modulo_list:
    ...
    for target_id1, decoder_id1 in zip(target_ids, outputs):
        id1_loss = torch.nn.functional.cross_entropy(decoder_id1.permute(0,2,1), target_id1, reduction='none', label_smoothing=0.05)
