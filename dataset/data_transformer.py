...
class TransformerDataDataset(torch.utils.data.Dataset):
    ...
    def add_noise(self, value):
        return value * (1 + 1e-2 * rng.normal(loc=0, scale=1, size=value.shape)) + 1e-2 * rng.normal(loc=0, scale=1, size=value.shape)
    ...
    def pad_output(self, text, feature):
        ...
        if p < 0.1:
            ...
            p = rng.uniform()
            input_codes[:] = np.where(rng.uniform(size=(max_decoderlen-1,)) < p, decoder_MSK, input_codes[:])
