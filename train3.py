...
def train():
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=1, factor=0.5, min_lr=1e-4)
    ...
    for epoch in range(last_epoch, EPOCHS):
        if denoise_epoch >= 0:
            training_dataset.noise_ratio = max(0.05, 0.75 ** (epoch - denoise_epoch))
        else:
        ...
        running_loss.reset()
        if denoise_epoch < 0 and loss_value < 3:
            denoise_epoch = epoch
